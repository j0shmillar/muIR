...
def check_ir_op_legality(op: Op, program: Program, cap: IRCapabilityDB) -> Tuple[bool, List[str]]:
    ...
    for c in variants:
        ...
        if c.inputs and len(op.inputs) != len(c.inputs):
            reasons.append(f"operand arity mismatch: got {len(op.inputs)}, want {len(c.inputs)}")
        else:
            for i, (tid, tc) in enumerate(zip(op.inputs, c.inputs)):
                ...
                if not _dtype_ok(t.type.dtype, tc):
                    reasons.append(f"operand[{i}] dtype {t.type.dtype} not in {tc.dtypes}")
                if not _rank_ok(len(t.type.shape), tc):
                    reasons.append(f"operand[{i}] rank {len(t.type.shape)} != {tc.rank}")
                if not _layout_ok(t.type.layout, tc):
                    reasons.append(f"operand[{i}] layout {t.type.layout} not in {tc.layouts}")

        if c.outputs and len(op.outputs) != len(c.outputs):
            reasons.append(f"result arity mismatch: got {len(op.outputs)}, want {len(c.outputs)}")
        else:
            for i, (tid, tc) in enumerate(zip(op.outputs, c.outputs)):
                ...
                if t is None:
                    ...
                    continue
                if not _dtype_ok(t.type.dtype, tc):
                    reasons.append(f"result[{i}] dtype {t.type.dtype} not in {tc.dtypes}")
                if not _rank_ok(len(t.type.shape), tc):
                    reasons.append(f"result[{i}] rank {len(t.type.shape)} != {tc.rank}")
                if not _layout_ok(t.type.layout, tc):
                    reasons.append(f"result[{i}] layout {t.type.layout} not in {tc.layouts}")
        ...
        for key, constraint in c.attrs.items():
            ...
            if isinstance(raw, list):
                if constraint.length is not None and len(raw) != constraint.length:
                    reasons.append(f"attr {key} length {len(raw)} != {constraint.length}")
            ...
            if constraint.one_of is not None:
                ...
                for v in seq:
                    if v not in constraint.one_of:
                        reasons.append(f"attr {key} value {v} not in {constraint.one_of}")
    ...
    best = min(variant_reasons, key=len) if variant_reasons else [f"no legal variant for {op.kind}"]
    return False, best
